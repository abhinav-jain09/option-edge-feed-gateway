...
Both bounds must fall between 18:00 ET on the prior calendar day minus one millisecond and 16:00 ET on the requested New York date. The read uses the existing locked `FootprintViews.barsPage`, with ascending keys and an atomic session identity. No new Kafka consumer, topic, REST endpoint or unbounded session buffer is created. Completeness is not asserted by an empty terminal page; Basic checks every elapsed RTH minute itself and uses premarket pages only for the chart and ladder.
...
| Correct session/cursor behavior | Canonical date and integral values; current ES-session bounds, ascending page boundaries and cross-session mismatch tested |
