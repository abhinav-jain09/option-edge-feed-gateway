...
| G-R10 | **Non-interference.** No CVD path changes when the flag is on except the one added hello field; flag off is byte-identical CVD behaviour (pinned: the hello JSON without footprint equals today's). A footprint record can never throw out of the consumer loop (every admission failure is a counted drop); the coordinator lock is never held while sending. **As-built (strike re-review round 2):** a socket's teardown never runs on the thread that enqueued for it: a channel that overflows is marked closed at once, and its session is closed on a shared closer thread, never on the enqueuing thread, so a close that blocks behind an outstanding write delays neither consumer nor any other socket. **As-built (round 3):** teardowns share ONE bounded pool — at most `CLOSER_THREADS` (4) closer threads exist at once, however many sockets overflow together, and a close that holds one past the close deadline is interrupted by the write watchdog; a teardown that cannot be handed over to the closers is never thrown at the enqueuer, so the frame still reaches every other socket; it stays pending, and the write watchdog retries it until the socket is torn down. **As-built (round 4):** a closer thread outlives every failure, because nothing replaces one: an `Error` out of a teardown never ends its closer thread, and a failure while reporting that failure never kills a closer thread either (the failure is counted before it is reported; a report that throws is swallowed and counted). A teardown whose session close throws is pending again, and the write watchdog retries it, so an accepted teardown is never stranded; `onClose` still runs exactly once. **As-built (round 5):** a hand-over never overwrites a closer's pending restore: the hand-over claims the teardown (PENDING → HANDED) before publishing it to the closers and writes nothing after, because a closer can run it, fail and put it back to PENDING before the hand-over's next step; every teardown-state write two threads can race is a compare-and-set from the state it leaves. |
...
| G-R3 | 6 of 6 probes pinned, over 6 clauses | 2 | re-run by the gate | Delivery and views |
...
| G-R10 | 10 of 10 probes pinned, over 10 clauses | 2 | re-run by the gate | Non-interference |
...
12 requirements; 11 probed by 50 mutations (49 killed, 1 surviving).
...
- **A slow socket stalls nothing (final re-review #1).** The fold's drainer fans each frame out to every socket's
  ...
  incremented) and its session close — then the detach callback — runs on a closer thread, never on the
  drainer: closing a WebSocket session can block behind the very write that made the client slow, and it used
  to block the live consumer inside the drainer, the cache consumer waiting for that drainer, its refusal
  control, and every healthy socket. This is `OutboundChannel`'s rule for every stream the gateway serves
  (overflow, write error and the write-deadline watchdog alike). Until the teardown runs the channel stays
  registered and closed, so no sender can fall back to writing to its session directly.
- **Teardowns share one bounded pool (final re-review round 3).** A thread per closing channel had no shared
  limit: a mass overflow started one per socket, and a `Thread.start()` that could not get a native thread threw
  `OutOfMemoryError` on the drainer — past the `RuntimeException` fallback, aborting that frame's fan-out after it
  had left the fold's outbox, with the channel closed and never torn down. The closers are now `CLOSER_THREADS`
  = 4 daemon threads `options-edge-ws-closer-N`, started once and shared by every channel; closing only queues
  the teardown (never blocks, never runs it inline, never starts a thread). Each channel queues its teardown at
  most once, so the queue holds at most one entry per socket that is closed and not yet torn down. A blocked
  close holds one closer thread and nothing else; the write watchdog interrupts a close that has held its thread
  past `CLOSE_DEADLINE_MS` (30 s, above Tomcat's 20 s blocking-send timeout, which already bounds a container
  close). When every closer thread is stuck, the other teardowns wait in the queue — their channels already
  closed, refusing every enqueue — and nothing else waits; a close that ignores both its timeout and the
  interrupt keeps its thread, and the pool goes on with one fewer. A teardown the closers cannot accept (an
  executor that rejects or throws an `Error`) is never thrown at the enqueuer: the frame still reaches every
  other socket, and the teardown stays pending until the write watchdog's sweep — which finds it, because the
  channel is still registered — hands it over. `onClose` still runs exactly once, after the session close.
- **No failure ends a closer thread (final re-review round 4).** A closer thread is never replaced, so a throw
  that escaped one was capacity lost for good: under heap exhaustion the failure's own log line threw
  `OutOfMemoryError` past the closer's catch, four such failures killed all four threads, and every accepted
  teardown — queued behind them or handed over later — waited forever, out of the watchdog's reach (it retries
  only a teardown that was never accepted). Each turn of a closer thread is now one try whose handler cannot
  throw: the failure is counted first (an increment that allocates nothing), then reported, and a report that
  throws is swallowed and counted as unreported. A teardown whose session close throws has not reached the
  detach: it is pending again, and the watchdog's next sweep hands it over again, so `onClose` still runs exactly
  once, after a close that returned. The hand-over's own failure report is guarded the same way, so it is thrown
  neither at the enqueuer nor at the watchdog.
- **A hand-over never overwrites a closer's pending restore (final re-review round 5).** The hand-over wrote
  HANDED after `execute()` returned, but by then a closer could already have run the teardown, had its session
  close throw, and put it back to PENDING. That write erased the PENDING, so the watchdog, which retries only a
  PENDING teardown, skipped the channel for good: the session was never closed again and `onClose` never ran, on
  every stream. The lifecycle is now NONE → PENDING → HANDED → DONE, with a failed close going HANDED → PENDING.
  The hand-over claims the teardown with the CAS PENDING → HANDED before publishing it and writes nothing after.
  A hand-over that throws goes back to PENDING only by the CAS HANDED → PENDING, so it never undoes what a closer
  already did. The closer's own restore clears `tornDown` first and then takes the CAS HANDED → PENDING, so a retry
  finds the teardown runnable. DONE, written once the close returned, is terminal. The pool bound, the 30 s close
  deadline, the watchdog retry and `onClose` exactly once are unchanged.
